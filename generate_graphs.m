function [Time, LVV, X] = generate_graphs(HR, Rs, Rm, Ra, Rc, Cr, Cs, Ca, Ls)
    
    if (strcmp(class(Cr),"char"))
        Cr = str2double(strrep(Cr,',','.'))
    end
    
    if (strcmp(class(HR),"char"))
        HR = str2double(strrep(HR,',','.'))
    end
    if (strcmp(class(Rs),"char"))
        Rs = str2double(strrep(Rs,',','.'))
    end
    if (strcmp(class(Rm),"char"))
        Rm = str2double(strrep(Rm,',','.'))
    end
    if (strcmp(class(Ra),"char"))
        Ra = str2double(strrep(Ra,',','.'))
    end
    if (strcmp(class(Rc),"char"))
        Rc = str2double(strrep(Rc,',','.'))
    end
    if (strcmp(class(Cs),"char"))
        Cs = str2double(strrep(Cs,',','.'))
    end
    if (strcmp(class(Ca),"char"))
        Ca = str2double(strrep(Ca,',','.'))
    end
    if (strcmp(class(Ls),"char"))
        Ls = str2double(strrep(Ls,',','.'))
    end
    ...
    %constants (we get these from the function parameters.
    %     HR = 60; %beats/min
    ...
    E(i) = (Emax - Emin)*En+Emin;
    LVV(i) = X(1,i)/E(i) + V0;
%   This is already being plotted on the graph within the app. 
%     plot(Time,X(1,:))
%     hold on
%     %figure
%     plot(Time,X(2,:))
